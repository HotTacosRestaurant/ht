const es = {
  nav: {
    home: "Inicio",
    menu: "Menú",
    locations: "Sucursales",
    reviews: "Reseñas",
    raffle: "Rifa",
    catering: "Catering",
    experience: "Experiencia",
    orderNow: "Ordenar",
    language: "EN",
    facebook: "Facebook",
    instagram: "Instagram",
  },

  hero: {
    eyebrow: "Comida Mexicana Auténtica",
    title: "HOT TACOS",
    subtitle: "Fiesta en cada mordida",
    body: "Comida mexicana auténtica, ambiente festivo y ordena rápido desde tu sucursal.",
    orderLeamington: "Ordenar Leamington",
    orderWindsor: "Ordenar Windsor",
    viewMenus: "Ver menús",
    helper: "Selecciona tu sucursal y ve directo a ordenar.",
  },

  quick: {
    menuTitle: "Menú",
    menuBody: "Leamington y Windsor tienen menús distintos.",
    locationsTitle: "Sucursales",
    locationsBody: "Llama, abre el mapa u ordena rápido.",
    raffleTitle: "Rifa",
    raffleBody: "Participa por beneficios reales.",
  },

  home: {
    locationsEyebrow: "Sucursales",
    locationsTitle: "Elige tu sucursal rápido",
    locationsDescription: "",
    menusEyebrow: "Menús",
    menusTitle: "Conoce nuestros menús",
    menusDescription: "",
    popularItems: "Platillos populares",
    orderLeamington: "Ordenar Leamington",
    orderWindsor: "Ordenar Windsor",
    viewMenu: "Ver menú",
    blockEyebrow: "Rifa + reseñas",
    blockTitle: "",
    blockDescription: "",
    seeReviews: "Ver reseñas",
    enterRaffle: "Entrar a la rifa",
  },

  menuPage: {
    eyebrow: "Menú",
    title: "Menú diferente por sucursal",
    description: "",
    fullMenuOrder: "Menú completo / Ordenar",
    branchDetails: "Detalles de sucursal",
  },

  locationsPage: {
    eyebrow: "Sucursales",
    title: "Leamington y Windsor",
    description: "Elige tu sucursal más cercana y toma el camino más corto para ordenar, llamar o abrir el mapa.",
  },

  branchPage: {
    leamingtonEyebrow: "Leamington",
    windsorEyebrow: "Windsor",
    description: "",
    orderOnline: "Ordenar en línea",
    directions: "Cómo llegar",
    call: "Llamar",
    viewMenu: "Ver menú",
    social: "Redes sociales",
    menuHighlightsLeamington: "Destacados del menú de Leamington",
    menuHighlightsWindsor: "Destacados del menú de Windsor",
  },

  cateringPage: {
    eyebrow: "Catering",
    title: "Comida para grupos, fiestas y eventos",
    description: "",
    body: "Pregunta por catering para cumpleaños, reuniones familiares, eventos de trabajo y celebraciones.",
  },

  footer: {
    brandText:
      "Comida mexicana auténtica, ambiente festivo y orden rápida en Leamington y Windsor.",
    quickLinks: "Enlaces rápidos",
    social: "Redes",
  },

  mobileBar: {
    orderOnline: "Ordenar",
    viewMenu: "Menú",
  },

  locationCard: {
    order: "Ordenar",
    directions: "Cómo llegar",
    call: "Llamar",
    details: "Detalles",
  },

  reviewsPage: {
    card1: "Gran sabor, porciones generosas y servicio rápido.",
    card2: "Sabe a casa. Un lugar perfecto para comida mexicana auténtica.",
    card3: "Buen valor, buenas porciones y ambiente festivo.",
  },
};

export default es;